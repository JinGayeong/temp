        ...
        <div class="container">
            <div class="search-window">
                <form action="">
                    ...
                    onclick="location.href = '${pageContext.request.contextPath}/info/QNAWrite.do'">1:1 문의 하기</button>
                </form>
            </div>
        </div>
    </div>
  ...
  <!-- board list area -->
        ...
        <div class="container">
            <table class="board-table">
                ...
                <tr>
                    ...
                    <th class="title">
                      <a href="readQNA.do?no=${qna.qnaNo}&pageNo=${qnaPage.currentPage}">
                      <c:out value="${qna.qnaTitle}" /><c:out value="${modQNAReq.title}" />
                      </a>
                    </th>
                    <td class="nickname">
                    ...
                    <c:if test="${qna.status == 0}">
                    	<td class="wating td-100">
                    		<c:if test="${authUser.rank == 3}">
                    			<a href="answerQNA.do?no=${qna.qnaNo}&pageNo=${qnaPage.currentPage}">답변대기</a>
                    		</c:if>
                    		<c:if test="${authUser.rank != 3}">
                    			답변대기
                    		</c:if>
	...
	<tr>
		<td colspan="4">
			...
			<a href="QNA.do?pageNo=${qnaPage.startPage - 5}">[이전]</a>
			</c:if>
			<c:forEach var="pNo" 
					   begin="${qnaPage.startPage}" 
					   end="${qnaPage.endPage}">
			<a href="QNA.do?pageNo=${pNo}">[${pNo}]</a>
			</c:forEach>
			<c:if test="${qnaPage.endPage < qnaPage.totalPages}">
			<a href="QNA.do?pageNo=${qnaPage.startPage + 5}">[다음]</a>
			</c:if>
		</td>
	</tr>
			</c:if>
